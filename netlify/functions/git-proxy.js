/* ---------------------------------------------------------------
   ...
   Remplace Git Gateway (toujours "Repository: Unknown").
   Valide le JWT Netlify Identity, puis proxifie vers l'API GitHub.
   --------------------------------------------------------------- */
const https = require('https');

const REPO  = 'pokechouu-netizen/delices-de-provence';
const BRANCH = 'main';
...
async function validateToken(token, siteUrl) {
  // Call the Netlify Identity endpoint to verify the token
  try {
...
exports.handler = async function (event, context) {
  ...
  /* Build GitHub URL
     Incoming path:  /.netlify/functions/git-proxy/data/products.json
     We want:        /repos/REPO/contents/data/products.json
  */
  ...
  const ghOptions = {
    ...
    headers: {
      ...
      'User-Agent': 'delices-de-provence-admin'
    }
  };
